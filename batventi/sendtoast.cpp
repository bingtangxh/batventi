...
int sendToast(int argc,char* argv[])
{
	...
	if(IsWindows8OrGreater()) {
		...
		if(!pSetCurrentProcessExplicitAppUserModelID) {
			...
			pSetCurrentProcessExplicitAppUserModelID(_appId);
			int method=EnsureShortcutWithAppID2();
			if(argc==2) {
...
BOOL TrySendToastDynamic(const wchar_t* message) {
	...
	hr=pWindowsCreateString(_appId,static_cast<UINT32>(wcslen(_appId)),&applicationId);
	if(FAILED(hr)) goto Cleanup;
...
DWORD EnsureShortcutWithAppID2(void)
{
	...
	return SUCCEEDED(CreateShortcutWithAppUserModelID2(shortcutPath,exePath,_appId)) ? 1 : 2;
