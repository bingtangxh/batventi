...
int msgbox(int argc, char *argv[])
{
	...
	// unsigned destLen = 0;
	if (argc < 1) {
		putsLFHy("Error from func msgbox in source file msgbox.c: Why argc < 1 ? I could not handle this.");
		return BAD_ARGC;
	}
	switch (argc)
	{
	...
	default:
	{
		...
		for (int i = 3; i < argc; i++) {
			...
			//	if (elemsGotten == 0) {
			//		putsHyphen("Warning from func msgbox in source file msgbox.c: Could not scan for CodePage from encoding parameter, I will use CP_ACP.");
			//		CodePage = CP_ACP;
			//	}
			//	continue;
			//}
			...
			/*if (specResult != NULL) {
				...
				case 0:
					...
					continue;
				default:
					printf("- Error from func msgbox in source file msgbox.c: Why errCode == %d while specResult == %p ?\n", errCode, specResult);
					return NOT_FOUND;
				}
			}
			else {
				switch (errCode) {
				case MALLOC_FAILED:
					putsHyphen("Error from func msgbox in source file msgbox.c: Func specifyParameter_multiple set errCode to MALLOC_FAILED");
					return MALLOC_FAILED;
				case NOT_FOUND:
					continue;
				default:
					printf("- Error from func msgbox in source file msgbox.c: Why errCode == %d while specResult == NULL ?\n", errCode);
					return NOT_FOUND;
				}
			}*/
