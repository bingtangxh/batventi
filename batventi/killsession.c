...
int KillSession_btv(int argc, char** argv) {
    HANDLE hToken=NULL;
    if (argc < 1) {
        putsLFHy("Error from func KillSession_btv in source file killsession.c: argc < 1 is unacceptable");
        return BAD_ARGC;
    }
    else {
        if (argc == 1 || (argc == 2 && !_strnicmp(argv[1], "help", 5)))
        {
            ...
            putsLFHy("Error from func KillSession_btv in source file killsession.c: Invalid argc, should be 1 or 2");
            return BAD_ARGC;
        }
        else if (!_strnicmp(argv[1], "ExitWindowsEx", 14)) {
            if(argc!=3) {
                putsLFHy("Error from func KillSession_btv in source file killsession.c: Invalid argc for ExitWindowsEx method, should be 2");
                return BAD_ARGC;
            }
            int elemsGotten = 0, flags = 0;
            elemsGotten = sscanf(argv[2], "%d", &flags);
            if (elemsGotten == 0) {
                putsLFHy("Error from func KillSession_btv in source file killsession.c: Could not scan for flags parameter, please provide a valid number for flags.");
                return 1;
            }
            ...
            if (!ExitWindowsEx(flags, 0)) {
                putsLFHy("Error from func KillSession_btv in source file killsession.c: Failed to call ExitWindowsEx");
                return 1;
            }
            else { return 0; }
            ...
            if (!ExitWindows(0, 0)) {
                putsLFHy("Error from func KillSession_btv in source file killsession.c: Failed to call ExitWindows");
                return 1;
            } else { return 0; }
        }
            ...
            // printf("%d\n", argc);
            if(argc!=3) {
                putsLFHy("Error from func KillSession_btv in source file killsession.c: Invalid argc for NtShutdownSystem method, should be 2");
                return BAD_ARGC;
            }
            int elemsGotten=0,mode=0;
            elemsGotten=sscanf(argv[2],"%d",&mode);
            if(elemsGotten==0) {
                putsLFHy("Error from func KillSession_btv in source file killsession.c: Could not scan for mode parameter, please provide a valid number for mode.");
                return 1;
            }
            if(getPrivilege(hToken,SE_SHUTDOWN_NAME)) { return 1; }
            ...
            putsLFHy("Error from func KillSession_btv in source file killsession.c: Failed to GetProcAddress for NtShutdownSystem");
            return 1;

        }
        else {
            putsLFHy("Error from func KillSession_btv in source file killsession.c: Invalid parameter");
            return 1;
        }
    }
}
...
int getPrivilege(HANDLE hToken, LPCWSTR privilegeName) {
    ...
    {
        putsLFHy("Error from func KillSession_btv in source file killsession.c: Failed to OpenProcessToken");
        return 1;
    }
    TOKEN_PRIVILEGES tkp;
    if (!LookupPrivilegeValue(NULL, privilegeName, &tkp.Privileges[0].Luid)) {
        putsLFHy("Error from func KillSession_btv in source file killsession.c: Failed to LookupPrivilegeValue for SE_SHUTDOWN_NAME");
        return 1;
    }
    ...
    if (!AdjustTokenPrivileges(hToken, FALSE, &tkp, 0, NULL, 0)) {
        putsLFHy("Error from func KillSession_btv in source file killsession.c: Failed to AdjustTokenPrivileges");
        return 1;
    }
    CloseHandle(hToken);
    return 0;
